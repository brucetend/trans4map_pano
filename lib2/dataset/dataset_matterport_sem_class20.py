...
class matterport_SemDataset33(data.Dataset):
    ...
    def __init__(self, root, fold, depth=True, hw=(512, 1024), mask_black=True, flip=False, rotate=False):
        ...
        for dname in __FOLD__[fold]:
            print("path_root:", root, dname, glob.glob(os.path.join(root, dname, 'rgb', '*jpg')),
                  sorted(glob.glob(os.path.join(root, dname, 'rgb', '*jpg'))))
            self.rgb_paths.extend(sorted(glob.glob(os.path.join(root, dname, 'rgb', '*jpg'))))
